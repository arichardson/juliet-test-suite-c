...
run_tests()
{
  ...
  for TESTCASE in $(ls -1 "${TYPE_PATH}"); do
    TESTCASE_PATH="${TYPE_PATH}/${TESTCASE}"
...
then
  for DIRECTORY in $(ls -1 "${SCRIPT_DIR}"); do
    ...
    then
      run_tests "${SCRIPT_DIR}/${DIRECTORY}" "good"
      run_tests "${SCRIPT_DIR}/${DIRECTORY}" "bad"
    fi
  done
elif [ -d "${SCRIPT_DIR}/CWE${TARGET}" ]
then
  run_tests "${SCRIPT_DIR}/CWE${TARGET}" "good"
  run_tests "${SCRIPT_DIR}/CWE${TARGET}" "bad"
else
